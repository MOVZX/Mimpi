// Z-Image Workflow Template (auto-generated from Pornmaster Z-Image Turbo_t2i_V2_magnification_fix_2026_01_19.json)
// Do not edit - regenerate from source workflow

const ZIMAGE_WORKFLOW_TEMPLATE = {
  "30": {
    inputs: {
      samples: ["63", 0],
      vae: ["100", 0],
    },
    class_type: "VAEDecode",
    _meta: {
      title: "VAE Decode",
    },
  },
  "63": {
    inputs: {
      add_noise: "enable",
      noise_seed: 899048185014980,
      steps: 4,
      cfg: 1,
      sampler_name: "euler",
      scheduler: "simple",
      start_at_step: 0,
      end_at_step: 9994,
      return_with_leftover_noise: "disable",
      model: ["110", 0],
      positive: ["90", 0],
      negative: ["109", 0],
      latent_image: ["152", 0],
    },
    class_type: "KSamplerAdvanced",
    _meta: {
      title: "KSampler (Advanced)",
    },
  },
  "90": {
    inputs: {
      text: "hyper-realistic full-body portrait of a stunning 36yo Asian woman",
      clip: ["99", 0],
    },
    class_type: "CLIPTextEncode",
    _meta: {
      title: "正面提示詞 (positive)",
    },
  },
  "98": {
    inputs: {
      unet_name: "Z-Image/beyondREALITY_V30.safetensors",
      weight_dtype: "default",
    },
    class_type: "UNETLoader",
    _meta: {
      title: "Load Diffusion Model",
    },
  },
  "99": {
    inputs: {
      clip_name: "Z-Image/qwen_3_4b_uncensored_aggressive.safetensors",
      type: "lumina2",
      device: "default",
    },
    class_type: "CLIPLoader",
    _meta: {
      title: "Load CLIP",
    },
  },
  "100": {
    inputs: {
      vae_name: "Z-Image/Z-Image VAE.safetensors",
    },
    class_type: "VAELoader",
    _meta: {
      title: "Load VAE",
    },
  },
  "109": {
    inputs: {
      conditioning: ["90", 0],
    },
    class_type: "ConditioningZeroOut",
    _meta: {
      title: "ConditioningZeroOut",
    },
  },
  "110": {
    inputs: {
      shift: 3,
      model: ["98", 0],
    },
    class_type: "ModelSamplingAuraFlow",
    _meta: {
      title: "ModelSamplingAuraFlow",
    },
  },
  "116": {
    inputs: {
      upscale_method: "bislerp",
      scale_by: 1.2,
      samples: ["147", 0],
    },
    class_type: "LatentUpscaleBy",
    _meta: {
      title: "Upscale Latent By",
    },
  },
  "118": {
    inputs: {
      samples: ["120", 0],
      vae: ["100", 0],
    },
    class_type: "VAEDecode",
    _meta: {
      title: "VAE Decode",
    },
  },
  "120": {
    inputs: {
      seed: 589631380207565,
      steps: 4,
      cfg: 1,
      sampler_name: "euler",
      scheduler: "simple",
      denoise: 0.65,
      model: ["110", 0],
      positive: ["90", 0],
      negative: ["109", 0],
      latent_image: ["116", 0],
    },
    class_type: "KSampler",
    _meta: {
      title: "KSampler",
    },
  },
  "147": {
    inputs: {
      anything: ["63", 0],
    },
    class_type: "easy cleanGpuUsed",
    _meta: {
      title: "Clean VRAM Used",
    },
  },
  "152": {
    inputs: {
      width: 896,
      height: 1152,
      batch_size: 1,
    },
    class_type: "EmptySD3LatentImage",
    _meta: {
      title: "EmptySD3LatentImage",
    },
  },
  "157": {
    inputs: {
      filename_prefix: "ComfyUI",
      subdirectory_name: "",
      output_format: "png",
      quality: "max",
      metadata_scope: "workflow_only",
      include_batch_num: true,
      prefer_nearest: true,
      images: ["118", 0],
    },
    class_type: "SaveImageWithMetaData",
    _meta: {
      title: "Save Image With MetaData",
    },
  }
};

export default ZIMAGE_WORKFLOW_TEMPLATE;
